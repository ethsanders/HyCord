...
SOURCE_CODE_LINK = "https://github.com/ProfessorPiggos/HyCord/"
...
def main():
    ...
    if NOTIFICATIONS:
        ...
        ), "API Key must be changed from default value if notification service is on."
        GAMESLIST = [
            "join",
            "leave",
            "BEDWARS",
        ...
        jsonpath = os.path.join(os.path.dirname(__file__), r"data/data.json")
        try:
            with open(jsonpath, "r") as listfile:
            ...
            jslist = {"track": {}, "listcmd": {}, "settings": {}, "online": {}}
            logging.info("JSON File not found. Creating one.")

    try:
        OWNERID = int(
            os.getenv("OWNERID")
        )  # No assertion as error will already caused if there is str to int conversion.
        OWNERFEATURES = True
    except:
        OWNERFEATURES = False
    ...
    class Notifications(
        commands.Cog
    ):  # Notification service, can be disabled with notifications.txt
                ...
                if jslist["online"][a]["trueonline"]:
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            f"https://api.hypixel.net/status?key={APIKEY}&uuid={a}"
                        ) as r:
                            if r.status == 200:
                                js = await r.json()
                                if not js["session"]["online"]:
                                    ...
                                    jslist["online"][a]["game"]
                                    != js["session"]["gameType"]
                                    and js["session"]["gameType"] != "MAIN"
                                    and js["session"]["gameType"] != "LIMBO"
                                    and js["session"]["mode"] != "LOBBY"
                                ):
                                    jslist["online"][a]["game"] = js["session"][
                                        "gameType"
                                    ]
                                    jslist["online"][a]["mode"] = js["session"]["mode"]
                                    game = USERGAMESLIST[
                                        GAMESLIST.index(jslist["online"][a]["game"])
                                    ]
                                    mode = jslist["online"][a]["mode"]
                                    for b in jslist["track"][a]:
                                        if jslist["settings"][str(b)][
                                            js["session"]["gameType"]
                                        ]:
                                            user = await bot.fetch_user(b)
                                            await user.send(
                                                f'**{curname}** joined the game "{game}". They are currently in the mode "{mode}".'
                                            )
        ...
        async def check(self) -> None:
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            ...
                            if r.status == 200:
                                ...
                                if js["session"]["online"]:
                                    ...
                                    async with aiohttp.ClientSession() as session2:
                                        async with session2.get(
                                            f"https://api.mojang.com/user/profiles/{a}/names"
                                        ) as t:
                                            if t.status == 200:
                                                js2 = await t.json()
                                    ...
                                    async with aiohttp.ClientSession() as session2:
                                        async with session2.get(
                                            f"https://api.hypixel.net/player?key={APIKEY}&uuid={a}"
                                        ) as t:
                                            if t.status == 200:
                                                js3 = await t.json()
                                                # try:
                                                if js3["player"]["lastLogin"] >= (
                                                    unixtime - 44500
                                                ):  # 44.5 seconds, stops double notifs
                                                    curname = js3["player"][
                                                        "displayname"
                                                    ]
                                                    jslist["online"][a] = {}
                                ...
                                if (
                                    js["session"]["online"]
                                    and not self.onlinecheck.is_running()
                                ):
            ...
            if dumpcount == 13:
                dumpcount = 0
                async with aiofiles.open(jsonpath, "w") as outfile:
        ...
        @commands.command(
            ...
            brief="Shows a list of all players on your notification list that are currently online",
        )
        ...
        @commands.command(
            aliases=["add", "notif", "addnotification"],
            brief="Adds to your notification list",
        )
        async def addnotif(self, ctx, arg) -> None:
            maxnotifsize = 10
            if not str(ctx.message.author.id) in jslist["settings"]:
                await ctx.send(
                    f"You need to run {PREFIX}notifsettings before adding notifications. Run {PREFIX}notifsettings without any arguments for setup instructions."
                )
                return
            if (
                str(ctx.message.author.id) in jslist["listcmd"]
                and len(jslist["listcmd"][str(ctx.message.author.id)]) >= maxnotifsize
                and ctx.message.author.id != OWNERID
            ):
                owner = await bot.fetch_user(OWNERID)
                await ctx.send(
                    f"Sorry, your user notification limit of {maxnotifsize} has been reached. Either remove someone from your notification list, or ask {owner} to increase the limit."
                )
                return
            if len(jslist["track"]) <= 32:
                if re.match(r"^[A-Za-z0-9_]+$", arg):
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            f"https://api.mojang.com/users/profiles/minecraft/{arg}"
                        ) as r:
                            if r.status == 200:
                                js = await r.json()
                                uuid = js["id"]
                                ...
                                await ctx.send(
                                    "Sorry, this player does not exist or the API is down."
                                )
                                return
                    ...
                    if not uuid in jslist["track"]:
                        ...
                        jslist["listcmd"][str(ctx.message.author.id)].append(
                            str.casefold(arg)
                        )
                        await ctx.send(
                            "You have been added to the notification list for "
                            + arg
                            + "."
                        )
                        ...
                        await ctx.send(
                            f"You are already on the notification list for {arg}."
                        )
                else:
                ...
                await ctx.send(
                    f"Sorry, the global notification list is full. Please contact {user} to let them know."
                )

        @commands.command(
            aliases=["remove", "removenotif", "removenotification"],
            brief="Removes from your notification list",
        )
        async def delnotif(self, ctx, arg) -> None:
            ...
            try:
                if int(ctx.message.author.id) in jslist["track"][uuid]:
                    ...
                    jslist["listcmd"][str(ctx.message.author.id)].remove(
                        str.casefold(arg)
                    )
                    await ctx.send(
                        f"You have been removed from the notificiation list for {arg}"
                    )
        ...
        @commands.command(
            aliases=["list", "listnotification", "listnotifications"],
            brief="Lists your notifications",
        )
        async def listnotif(self, ctx) -> None:
            try:
                if len(jslist["listcmd"][str(ctx.message.author.id)]) != 1:
                    output = (
                        "**"
                        + str(len(jslist["listcmd"][str(ctx.message.author.id)]))
                        + " players:** "
                    )

                    for i in range(
                        len(jslist["listcmd"][str(ctx.message.author.id)]) - 1
                    ):
                        currentname = jslist["listcmd"][str(ctx.message.author.id)][i]
                        output += f"{currentname}, "
                    else:
                        currentname = jslist["listcmd"][str(ctx.message.author.id)][
                            len(jslist["listcmd"][str(ctx.message.author.id)]) - 1
                        ]
                        output += f"and {currentname}."
                else:
        ...
        async def notifsettings(self, ctx, *args) -> None:
            ...
            if args == ():
                ...
                await ctx.send(
                    f"Settings have sucessfully been set. You can now use the {PREFIX}addnotif command, or use this command again."
                )
            elif len(args) == 1:
                ...
                if argument == "joinonly":
                    ...
                    await ctx.send(
                        f"Only join notifications will be sent. You can now use the {PREFIX}addnotif command."
                    )
                if argument == "allon":
                    ...
                    await ctx.send(
                        f"All notifications will be sent. You can now use the {PREFIX}addnotif command."
                    )
                ...
                await ctx.send(
                    f"You didn't have the correct amount of arguments. You should have {len(GAMESLIST)} arguments. Everything set to on."
                )
    ...
    async def getstatus(ctx, arg) -> None:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                "https://api.slothpixel.me/api/players/" + arg + "/status"
            ) as r:
                if r.status == 200:
                    js = await r.json()
                    if js["online"]:
    ...
    @bot.command(
        aliases=["players", "hypixelplayers"], brief="Gets Hypixel player count"
    )
            ...
            async with session.get("https://api.slothpixel.me/api/counts") as r:
                if r.status == 200:
                    ...
                    await ctx.send(
                        "There are currently "
                        + str(js["playerCount"])
                        + " players online on Hypixel."
                    )

    @bot.command(
        aliases=["playerguild", "findguild", "guildbyplayer"],
        brief="Gets info of a guild by player name",
    )
                    ...
                    if js["description"] != None:
                        ...
                        if len(js["preferred_games"]) >= 3:
                            await ctx.send(
                                ...
                                + ". "
                                + js["name"]
                                + " has a guild level of "
                                + str(js["level"])
                                + ', and their guild description is "'
                                + desc
                                + '". '
                                + "Their top 3 games are "
                                + js["preferred_games"][0]
                                + ", "
                                + js["preferred_games"][1]
                                + ", and "
                                + js["preferred_games"][2]
                                + "."
                            )
                        else:
                            await ctx.send(
    ...
    @bot.command(
        aliases=["guild", "guildsearch", "guildbyname"],
        brief="Gets info of a guild by guild name",
    )
    async def guildinfo(ctx, *, arg) -> None:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                "https://api.slothpixel.me/api/guilds/name/" + arg
            ) as r:
                if r.status == 200:
                    js = await r.json()
                    if js["description"] != None:
    ...
    async def botinfo(ctx) -> None:
        await ctx.send(
            f"**This bot has many features related to the Hypixel API. Use {PREFIX}help to look through all of the commands you can use. \n The source code for the bot is available at **{SOURCE_CODE_LINK}**. ProfessorPiggos is the main developer on the project."
        )
    ...
    if OWNERFEATURES:
        ...
        @bot.command(
            aliases=["s", "stopbot"],
            brief="Stops the bot. Only the host of the bot can use this.",
        )
        async def stop(ctx) -> None:
            if int(ctx.message.author.id) == OWNERID:
                async with aiofiles.open(jsonpath, mode="w") as outfile:
                    json.dump(jslist, outfile, indent=6)
                    if Notifications.check.is_running():
                        ...
                        while Notifications.check.is_running():
                            ...
                            if loopcount1 >= 48:
                                await ctx.send(
                                    "**This is taking a while, but the bot is still running.**"
                                )
                                loopcount1 = 0
